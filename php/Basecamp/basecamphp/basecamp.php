...
class Basecamp {
	...
	function hook($url, $expected, $post = false, $get = false, $method = false) {
		$returned = $this->unserialize($this->request($url, $post, $get, $method));
		$placement = $expected;
	...
	function request($url, $post = false, $get = false, $method = false) {
		if($get)
		{
		...
		switch($method)
		{
			case 'get'   : $req->setMethod(HTTP_REQUEST_METHOD_GET);    break;
			case 'put'   : $req->setMethod(HTTP_REQUEST_METHOD_PUT);    break;
			case 'delete': $req->setMethod(HTTP_REQUEST_METHOD_DELETE); break;
			case 'post'  : $req->setMethod(HTTP_REQUEST_METHOD_POST);   break;
			default:
		}
